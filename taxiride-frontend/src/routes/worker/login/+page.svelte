<script lang="ts">
  ...
  async function handleLogin() {
    ...
    try {
      await workerService.login(login, password);
  ...
  async function handleRegister() {
    ...
    try {
      ...
      await workerService.login(registerData.login, registerData.password);
      goto('/worker/dashboard');
    } catch (err: unknown) {
...
<svelte:head>
  <title>Logowanie Pracownika - TaxiRide</title>
</svelte:head>
  ...
  <div class="max-w-md w-full space-y-8">
    <div>
      <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900 dark:text-white">
        {showRegister ? 'Rejestracja Pracownika' : 'Logowanie Pracownika'}
      </h2>
      <p class="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
        {showRegister ? 'Utwórz konto pracownika' : 'Zaloguj się do panelu pracownika'}
      </p>
    </div>
    ...
    {#if showRegister}
      ...
      <form class="uber-card space-y-6" on:submit|preventDefault={handleRegister}>
        ...
        <div>
          <label for="department" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Dział</label>
          <select
            id="department"
